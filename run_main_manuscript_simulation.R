...
#' Harrell's Concordance Index (Harrell's C-index), and Uno's Concordance Index (Uno's C-index) and optionally generates performance boxplots.
#'
...
#' @param parallel.rho Logical; enable parallel processing over rho (default: FALSE)
#' @param parallel.cv Logical; enable parallel processing over cv (default: TRUE)
#' @param ncore_max_rho Maximum number of cores for parallel processing for splitting over rho_values (default: 2)
...
#' 
#' @return This function does not return a value, but it writes performance metrics (`C-index`, `FPR`, `FNR`, `NSR`) to text files
...
#' It uses the R package parallel and supports the nested parallelization over rho and cross-validation.
...
#' @examples
#' \dontrun{
#' run_main_manuscript_simulation(nsimul = 5, model = "weibull", tu = 6, tz = 6,
#'                           weak = TRUE, rate = 40, lam_min = FALSE,
#'                           snr = 0.8,     
#'                           parallel.rho= TRUE,ncore_max_rho=2,
#'                           parallel.cv= TRUE,ncore_max_cv=5,
#'                           folder = "MAIN_SIMULATION_OUTPUT", iplot_box = TRUE)
#' }
#'
#' @import survival
#' @import Hmisc
...
#' 
run_main_manuscript_simulation<-function(nsimul,model,tu,tz,weak=T,rate=40,lam_min =F,snr = snr, 
                                    parallel.rho= TRUE,
                                    ncore_max_rho=2,
                                    parallel.cv= TRUE,
                                    ncore_max_cv=5,
                                    folder="OUTPUT", 
                                    iplot_box=T,iplot_CV=F){
  myseed=123
  set.seed(myseed)
  library(survival)
  library(Hmisc)
  ...
  C.index_uno <- matrix(0,nsimul,n_rho_values) # UNO C index
  ...
  for(isimul in c(1:nsimul)){
    ...
    # definition of tau constant for Uno C-index
   ...
   # print(paste("est sigma only intercept", sigma.est))
    ...
    C.index_uno[isimul,1]<- survAUC::UnoC(Surv(Y_train, delta_train), Surv(Y_test, delta_test), -predicted,time =unotimes)
    print((paste0('UNO C-index = ',C.index_uno[isimul,1])))
    ...
    ## Uno C-index from survAUC package 
    C.index_uno[isimul,2]<- survAUC::UnoC(Surv(Y_train, delta_train), Surv(Y_test, delta_test), -predicted,time =unotimes)
    print((paste0('UNO C-index = ',C.index_uno[isimul,2])))
    ...
    for(i_rho in c(1:(n_rho_values-2))){
      ...
      ## Uno C-index from survAUC package
      C.index_uno[isimul,2+i_rho]<- survAUC::UnoC(Surv(Y_train, delta_train), Surv(Y_test, delta_test), -predicted,time =unotimes)
      print((paste0('UNO C-index = ',C.index_uno[isimul,2+i_rho])))
  ...
  colnames(C.index_uno)<-c("only U","only Z","early fusion",paste0("rho=",rho_values[2:(n_rho_values-2)]))
  file_out=paste0(dir_out,"Cindex_Uno_nsimul_",nsimul,"_model_",model,"_weak_",weak,"_tu_",tu,"_tz_",tz,"_rate_",rate,"_lam_min_", lam_min,"_SNR_",snr,".txt")
  write.table(C.index_uno, file = file_out, sep = "\t", row.names = F, col.names = T)
  ...
  print(paste0("Running time (simulation setting) : ",End_time-Start_time)) 
  ...
  if (iplot_box){
    ...
    file_in <- paste0(dir_input,"Cindex_nsimul_",nsimul,"_model_",model,"_weak_",weak,"_tu_",tu,"_tz_",tz,"_rate_",rate,"_lam_min_", lam_min,"_SNR_",snr,".txt")
    Cindex <- read.table(file = file_in, sep = "\t",header = TRUE,check.names=FALSE)
    
    file_in <- paste0(dir_input,"Cindex_Uno_nsimul_",nsimul,"_model_",model,"_weak_",weak,"_tu_",tu,"_tz_",tz,"_rate_",rate,"_lam_min_", lam_min,"_SNR_",snr,".txt")
    C.index_uno <- read.table(file = file_in, sep = "\t",header = TRUE,check.names=FALSE)
    
    file_in<-paste0(dir_input,"FPR_nsimul_",nsimul,"_model_",model,"_weak_",weak,"_tu_",tu,"_tz_",tz,"_rate_",rate,"_lam_min_", lam_min,"_SNR_",snr,".txt")
    FPR=read.table(file = file_in, sep = "\t",header = TRUE,check.names=FALSE)
    
    file_in<-paste0(dir_input,"FNR_nsimul_",nsimul,"_model_",model,"_weak_",weak,"_tu_",tu,"_tz_",tz,"_rate_",rate,"_lam_min_", lam_min,"_SNR_",snr,".txt")
    FNR=read.table(file = file_in, sep = "\t",header = TRUE,check.names=FALSE)
    
    file_in <- paste0(dir_input,"NSR_nsimul_",nsimul,"_model_",model,"_weak_",weak,"_tu_",tu,"_tz_",tz,"_rate_",rate,"_lam_min_", lam_min,"_SNR_",snr,".txt")
    NSR <- read.table(file = file_in, sep = "\t",header = TRUE,check.names=FALSE)
    
    tabComplete <- round(rbind(apply(Cindex,2,mean),apply(C.index_uno ,2,mean),apply(FPR,2,mean),apply(FNR,2,mean),apply(NSR,2,mean)),3)
    tabComplete <- cbind(c("Harrell’s C-index","Uno's C-index","FPR","FNR","NSR"),tabComplete)
    
    colnames( tabComplete)[1]<-"Error/Method"
    print(tabComplete)
   
    stargazer(tabComplete,summary=FALSE)
    file_out<-paste0(dir_out,"Table_Err_nsimul",nsimul,"_model_",model,"_weak_",weak,"_tu_",tu,"_tz_",tz,"_rate_",rate,"_lam_min_", lam_min,"_SNR_",snr,".txt")
    
    write.table(tabComplete, file = file_out, sep = "\t", row.names = F, col.names = T,quote = F)
    
    bxp1 <- boxplot_measure(nsimul,Cindex,"Harrell’s C-index",c(0.4,1))
    bxp2 <- boxplot_measure(nsimul,FPR,"FPR")
    bxp3 <- boxplot_measure(nsimul,FNR,"FNR")
    bxp4 <- boxplot_measure(nsimul,NSR,"NSR")
    bxp5 <- boxplot_measure(nsimul,C.index_uno,"Uno's C-index",c(0.4,1))
    
    # Reduce spacing between panels
    bxp1 <- bxp1 + theme(plot.margin = margin(5,2,5,2))
    bxp2 <- bxp2 + theme(plot.margin = margin(5,2,5,2))
    bxp3 <- bxp3 + theme(plot.margin = margin(5,2,5,2))
    bxp4 <- bxp4 + theme(plot.margin = margin(5,2,5,2))
    bxp5 <- bxp5 + theme(plot.margin = margin(5,2,5,2))
    
    # Combined horizontal figure
    file_out2 <- paste0(dir_out, "Boxplot_nsimul_", nsimul, "_model_", model,
                        "_weak_", weak, "_tu_", tu, "_tz_", tz, "_rate_", rate,
                        "_lam_min_", lam_min, "_SNR_", snr, ".pdf")
    
    combined_plot2 <- grid.arrange(bxp1, bxp5, bxp2, bxp3, bxp4, ncol = 5, nrow = 1)
    
    ggsave(file_out2, plot = combined_plot2, width = 15, height = 4, device = cairo_pdf)
